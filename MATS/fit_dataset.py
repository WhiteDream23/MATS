...
class Fit_DataSet:
    ...
    为拟合生成参数的最小强度。 注意： 即使参数列表中的值是浮动的，如果低于此阈值，也不会成为浮动值。 默认值为 1e-27。
    ...
    def residual_analysis(self, result, indv_resid_plot = False):
        ...
        用拟合结果更新每个光谱对象中的模型和残差阵列，并为每个光谱提供生成吸收和残差组合图的选项。
    ...
    def update_params(self, result, base_linelist_update_file = None , param_linelist_update_file = None, 
        ...
        根据拟合结果更新基线和线参数文件，可选择写入文件（默认）或保存为新文件，并更新频谱对象中的基线值。
