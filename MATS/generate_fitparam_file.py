...
class Generate_FitParam_File:
    ...
    linemixing : bool
    如果为 False，则所有线条混合参数都设置为 0，不能浮动。
    ...
    nu_constrain : bool
    True 表示线中心将是所有光谱的全局变量。 假则会为数据集中的每个光谱生成一个新值。
    ...
    def generate_fit_param_linelist_from_linelist(self, vary_nu = {}, vary_sw = {},
        ...
        设置分子和同位素行中心是否应浮动的字典。 字典中的每个键都对应一个分子，值是一个字典，以 local_iso_ids 作为键，布尔值作为布尔标志。 true 表示浮动该分子和同位素的 nu。
        例如 vary_nu = {7:{1:True, 2: False, 3: False}，1:{1:False, 2: False, 3: False}}
        表示浮动所有主要氧同位素的谱线中心（如果谱线强度大于拟合强度），而所有其他氧同位素和所有水同位素的谱线中心不会变化
        如果这些值留空，则所有变量都必须手动切换为浮动。 默认值为 {}。
        ...
        根据参数行列表返回数据帧，并为每个可浮动参数添加变化列和错误列。
        变量列由输入和拟合强度值定义。 错误列将由拟合结果填充。 数据帧也将保存为 .csv 文件。
        线强将通过拟合强度（设置为 sw_scale_factor）进行归一化。
        sw "一词现在等于归一化值，因此在模拟中 "sw'*sw_scale_factor "将用于线强度。
        由于线强度非常小，因此很难在不进行归一化处理的情况下对其进行拟合。
        """
