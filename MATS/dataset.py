...
class Dataset:
    ...
    读入拟合时使用的参数行列表。 这样就可以检查输入光谱和参数行列表之间的一致性。
    ...
    baseline_order : int
    设置数据集中所有光谱的基线顺序。 这将自动设置为数据集中所有光谱的最大基线阶次。
    ...
    def generate_baseline_paramlist(self):
        ...
        生成名为 dataset_name + _baseline_paramlist 的 csv 文件,该文件将用于生成另一个 csv 文件
        用于拟合光谱相关参数，其列包括光谱编号、区段编号、x_shift、数据集中每个分子的浓度、基线项（a = 0 阶项，b = 1 阶项等）和等离子项（为数据集中每个光谱列出的等离子数设置振幅、周期和相位）。
        Returns
        -------
