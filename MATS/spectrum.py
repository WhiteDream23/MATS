...
class Spectrum:
    ...
    标称温度表示光谱的大致温度。 在将光谱合并到一个数据集时，这将作为一个标志，指示温度相关项是否应作为可以拟合的参数，或者是否应作为常数。
    ...
    频谱轴 x 偏移的波长值。 这是一个可拟合参数。 在使用该参数时要小心，因为浮动多个具有类似效果的参数会导致拟合不收敛，例如……
    ...
    分辨率（Resolution）是以波长为单位的 ILS 分辨率浮点数或数组。
    ...
    AF_wing 是一个浮点数或数组，包含以 cm-1 为单位的 ILS 计算范围。 对于 HAPI 狭缝函数来说，这是一个单一值，但对于用户自定义函数来说，可以是一个由多个值组成的数组。 默认值为 10 cm-1。
